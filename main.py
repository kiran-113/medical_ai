...
if st.button("Open Camera"):
    # Display the native camera input when button is clicked
    st.markdown("""
    <input type="file" accept="image/*" capture="environment" id="camera_input" style="display:none;" onchange="displayImage(this)">
    <script>
        ...
        function displayImage(input) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    const streamlitImage = document.getElementById('streamlitImage');
                    streamlitImage.src = img.src;
                    streamlitImage.style.display = 'block';
                };
...
elif st.session_state.get('camera_image') is not None:  # Check for camera image in session state
    image_source = st.session_state.camera_image
else:
    image_source = None
...
# api_key = "ygshjbkb=dwsddcs"  # Replace with your actual API key
# # Configure the API
# genai.configure(api_key=api_key)  # Uncomment this if using the genai library
...
# # Function to display the native camera input
# def display_native_camera():
#     st.markdown("""
#     <input type="file" accept="image/*" capture="environment" id="camera_input" style="display:none;">
#     <label for="camera_input" class="custom-file-upload" style="background-color: #008CBA; color: white; padding: 10px; border-radius: 10px; cursor: pointer;">Open Camera</label>
#     """, unsafe_allow_html=True)

# # Call the function to display the native camera input
# display_native_camera()

# # File uploader button only
# uploaded_file = st.file_uploader("Browse files", type=["jpg", "jpeg", "png"], label_visibility="collapsed")

# # Check if the inline camera input has been triggered by user interaction
# camera_image = st.camera_input("Click to take a picture for analysis")

# # Determine the image source
# if uploaded_file or camera_image is not None:
